...
campos([1.1363    0.1512    0.3664]);
camtarget([-0.0293    0.1040         0]);
tfactor = 1;
tic;
curr_time = 0;
...
ball_patch.FaceAlpha = 0.2;
for i = 1:3
    iiwa.link_patches{i}.FaceAlpha = 0.6;
end
...
for j = 1:5
for i = linspace(start_t - 0.4, end_t + .4, 60*3)
    joint_pos_struct = interpolate_traj(iiwa, all_jnt_breaks, all_jnt_angles, i);
    display_at_pose(iiwa, joint_pos_struct);
        quat_eval = interp1(tspan, quatspan, i);
    pos_eval = interp1(tspan, posspan, i);
