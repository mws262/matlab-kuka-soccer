...
if exist(saved_path_name, 'file') && load_saved_path
    ...
    load(saved_path_name);    segs_between = 3; % One of the most important parameters. Will change the number of extra segments between required knot points.
...
if write_to_vid
    framerate = 30;
    vid_writer = VideoWriter(['../../../data/videos/raw/', 'pushing_plane_8_colored_plane.avi']); % Convert for Slack with 'ffmpeg -i infile.avi youroutput.mp4'
...
camposes = [    0.0433   -0.0065    1.5584;
    0.0433   -0.0065    1.5584;
    
    0.5808    0.9923    0.8559;
    0.5808    0.9923    0.8559;
    
    -0.3406    1.4541    0.6847;
    -0.3406    1.4541    0.6847;
    
    -1.2605   -1.1008    0.7366;
    -1.2605   -1.1008    0.7366];

camtimes = [0; 2; 3; 5; 6; 8; 9; 11];
