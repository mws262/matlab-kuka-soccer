function spline_optim = nlp_spline_hax(user_problem, user_options)
...
vel_generic_x = poly_dt_fcn(t_p, a);
vel_generic_y = poly_dt_fcn(t_p, b);
cost_integral = 0;
...
%         int(accel_generic_x(i-1).^2 + accel_generic_y(i-1).^2, t_p, 0 ,te(i-1) - tst(i-1));
    ...
    effort_term = int(vel_generic_x(i).^2, t_p, 0 ,te(i) - tst(i)) + int(vel_generic_y(i).^2, t_p, 0 ,te(i) - tst(i));
    cost_integral = cost_integral + effort_term;
end

% Enforce that some points MUST pass through pinned positions.
pinned_pts_x_cost = sum(([xst(1:problem.polys_per_pinned_knot:end); xe(end)] - problem.pinned_knots(:,1)).^2);
pinned_pts_y_cost = sum(([yst(1:problem.polys_per_pinned_knot:end); ye(end)] - problem.pinned_knots(:,2)).^2);
...
effort_cost = 0.0001*sum(0*all_coeffs(1:4:end).^2 + all_coeffs(2:4:end).^2);
cost_fun = matlabFunction(cost_integral + (1 - problem.adjacent_segment_product_scaling)*effort_cost, 'Vars', {all_coeffs});
pos_penalty_fun = matlabFunction(pinned_pts_x_cost + pinned_pts_y_cost + 0.001*cost_integral, 'Vars', {all_coeffs});
...
[sol, ~] = fmincon(pos_penalty_fun, x0, [],[], A_num, B_num,[],[],[], options.fminopts);
