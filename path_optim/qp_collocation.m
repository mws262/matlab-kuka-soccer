% Desired knot points.
knots = [0, 0; 0.5, 0.5; 0, 1; -0.5, 0.5; 0, 0; 0.5, -0.5; 0, -1; -0.5, -0.5; 0, 0];
segments = size(knots,1) - 1;
breaks = 0:1:segments;
% Insert collocation points (not fixed) between each defined knot point.
extraPointsPerSegment = 5;
...
velocityWeight = 1; % Weight for velocity vs acceleration terms in cost.
...
for i = 0:numColPts - 2
    ...
    vHalfCol(i + 1, vStartIdx + i) = 1;
    vHalfCol(i + 1, vStartIdx + i + 1) = 1;
...
tic
[X, fval] = quadprog(costHboth, [], [], [], Aboth, bboth, [], [], [], []);
toc
...
pp = ppmak(allTimes, coefs, 2);

solEval = ppval(pp, linspace(allTimes(1), allTimes(end), 1000))';
