function derived_eqns = do_derivations()
    ...
    derived_eqns.friction_required_fcn = matlabFunction(friction_required);
    derived_eqns.x_force_required_fcn = matlabFunction(fax_solve); % Required x component of force applied by arm.
    derived_eqns.y_force_required_fcn = matlabFunction(fay_solve);
    derived_eqns.angular_rate_wx_fcn = matlabFunction(wx);
    derived_eqns.angular_rate_wy_fcn = matlabFunction(wy);
    derived_eqns.contact_arc_fcn = matlabFunction(full_contact_arc_shifted');
    derived_eqns.contact_arc_centered_fcn = matlabFunction(full_contact_arc');
    derived_eqns.equator_contact_velocity_fcn = matlabFunction(equator_contact_velocity);
    derived_eqns.world_contact_velocity_fcn = matlabFunction(vsurf_general);
    derived_eqns.v_surfx_fcn = matlabFunction(v_surfx);
    derived_eqns.v_surfy_fcn = matlabFunction(v_surfy);
    derived_eqns.isurf_fcn = matlabFunction(isurf');
    derived_eqns.jsurf_fcn = matlabFunction(jsurf');    
end
