...
exceptions{end+1} = do_test(@end_conditions_met);
    ...
    function end_conditions_met()
        ...
        assert_near(ppval(accel, accel.breaks(1)), [0; 0; 0], tolerance, 'Start accel does not match zero.');
        assert_near(ppval(vel, vel.breaks(end)), [0; 0; 0], tolerance, 'End vel does not match zero.');
        assert_near(ppval(accel, accel.breaks(end)), [0; 0; 0], tolerance, 'End accel does not match zero.');
    end
end
